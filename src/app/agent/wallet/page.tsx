...
import {
  Wallet, ArrowUpRight, ArrowDownLeft, Loader2, Plus, Building2,
...
export default function AgentWalletPage() {
  ...
  const resetAddForm = () => {
    setAddOpen(false); setBankCode(""); setAccountNumber(""); setResolvedName(null); setResolveError(null)
  }
  ...
  const setDefaultMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Default account updated"); queryClient.invalidateQueries({ queryKey: ["bank-accounts"] }) },
  })

  const removeBankMutation = useMutation({
    mutationFn: (id: string) => paymentsApi.removeBankAccount(id),
    onSuccess: () => { toast.success("Bank account removed"); queryClient.invalidateQueries({ queryKey: ["bank-accounts"] }) },
  })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Your commission earnings and withdrawals</p>
      </div>
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                Complete identity verification to withdraw your commission earnings to your bank.
              </p>
            </div>
          </div>
          <Link
          ...
          <Card className="bg-gradient-to-br from-[#1a3c5e] to-[#0f2d48] text-white border-0">
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="p-2.5 rounded-xl bg-white/20"><Wallet className="h-5 w-5" /></div>
              ...
              <p className="text-xs text-slate-300 mt-3">Commissions paid by landlords land here.</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                {withdrawAmount && !balanceSufficient && (
                  <p className="text-xs text-red-600 mt-1.5">Amount exceeds balance or below ₦1,000 minimum</p>
                )}
              </div>
              <div>
          ...
          <Card>
            ...
            <CardContent>
              {txLoading ? (
                ...
                <EmptyState icon={Wallet} title="No activity yet" description="Commission payouts and withdrawals will appear here." />
              ) : (
                      ...
                      <div key={tx.id} className="flex items-center justify-between px-3 py-3 rounded-xl hover:bg-slate-50 transition-colors">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`p-2 rounded-xl ${isCredit ? "bg-emerald-50" : "bg-slate-100"}`}>
                            {isCredit ? <ArrowDownLeft className="h-4 w-4 text-emerald-600" /> : <ArrowUpRight className="h-4 w-4 text-slate-600" />}
                          </div>
                  ...
                  {bankCode && accountNumber.length === 10 && (
                    <div
                      className={`rounded-lg px-3 py-2.5 flex items-center gap-2.5 text-sm border ${
                        resolving ? "bg-slate-100 border-slate-200 text-slate-600"
                        : resolvedName ? "bg-emerald-50 border-emerald-200 text-emerald-800"
                        : resolveError ? "bg-red-50 border-red-200 text-red-800"
              ...
              {banksLoading ? (
                ...
                banks.map((bank) => (
                  <div key={bank.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-200 hover:border-slate-300 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="p-2 rounded-lg bg-[#1a3c5e]/10 shrink-0"><Building2 className="h-4 w-4 text-[#1a3c5e]" /></div>
                      <div className="min-w-0">
